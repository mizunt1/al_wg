...
class ActiveLearningDataSources():
    def __init__(self, datasets, dataset_val,
                 dataset_test, num_workers=4, batch_size=64, batch_size_test=64):
        if isinstance(datasets, list):
            ...
            self.dataset_val = torch.utils.data.ConcatDataset(dataset_val)
            self._create_source_indices()
        else:
    ...
    def _create_source_indices(self):
    ...
    def get_indices_one_source(self, source=0, size=10):
        # pool indices from that source
        indexes_for_source = [i for i in range(self.source_idx[source], self.source_idx[source+1])]
        indexes_in_pool = self.pool.indices
        available_from_source = list(set(indexes_for_source).intersection(set(indexes_in_pool)))
        assert size <= len(available_from_source) , f"for source {source} there is no longer {size} data points available. Only {len(available_from_source)} available."
        if size!= -1:
            available_indices = np.random.permutation(available_from_source)[:size]
        else:
            available_indices = np.random.permutation(available_from_source)
        return available_indices

    def get_indices_sources(self, source_dict={0:0,1:10}):
        indices = []
        for source, size in source_dict.items():
            indx = self.get_indices_one_source(source=source, size=size)
            indices.extend(indx)
        return indices
    ...
    def get_train_and_test_and_val_loader(self, batch_size):
        return (DataLoader(self.train, batch_size=self.batch_size,
                           num_workers=self.num_workers, pin_memory=True, shuffle=True),
                DataLoader(self.dataset_val, batch_size=self.batch_size_test, shuffle=True),
