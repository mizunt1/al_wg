...
METHODS=(uniform_groups)
N_METHODS=${#METHODS[@]}
N_SEEDS=5
SEED=$(( SLURM_ARRAY_TASK_ID % N_SEEDS))
METHOD_IDX=$(( (SLURM_ARRAY_TASK_ID / N_SEEDS) % N_METHODS ))
python cmnist_al.py --project_name leaky_groups --seed $SEED --data_mode leaky_groups --al_size 30 --acquisition ${METHODS[$METHOD_IDX]}  --start_acquisition uniform_groups --causal_noise 0.0 --spurious_noise 0.01 --num_spurious_groups 4
