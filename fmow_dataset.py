...
class FMoWDataset(WILDSDataset):
    ...
    def __init__(self, version=None, root_dir='data', download=False, split_scheme='official', seed=111, use_ood_val=True):
        self.group_string_map = {str(key): key for key in range(0,5)}
        ...
        if self._split_scheme.startswith('time_after'):
            ...
            self.val_ood_mask = np.asarray(
                pd.to_datetime(self.metadata['timestamp'], format='ISO8601') >= year_minus_3_dt) & ~self.test_ood_mask
    ...
    def list_number_of_points_per_source(self):
        """
        Args:
...
if __name__ == "__main__":
    data = FMoWDataset(root_dir='/network/scratch/m/mizu.nishikawa-toomey', download=True)
    rng_state = np.random.get_state()
    
    # Print regions in each split
    for split_name in data._split_dict.keys():
        subset = data.get_subset(rng_state, split_name)
        import pdb
        pdb.set_trace()
        regions = np.unique(data.metadata.iloc[subset.indices]['year'])
        print(f"Split: {split_name}, Regions: {regions}")
